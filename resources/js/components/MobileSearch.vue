<template lang="pug">
	form(name="header-search-form-mobile" method="get" :action="'/' + lang + '/search-results'")
		.header__search
			.container
				.form-group
					label.form-label.form-label_hidden(for="header-search-mobile") {{ __('Search box') }}
				...
				.results-panel.bg_white(v-if="!$v.$invalid")
					.results-panel__content.scrollbar-inner(
						@touchmove="scrollResults($event)"
						@scroll="scrollResults($event)"
						@wheel="scrollResults($event)"
					)
									...
									.item-results__cell.item-results__cell_img
										...
										:href="product.link")
											picture
												...
												img(v-if="product['imagesResized'] != undefined && product['imagesResized'].length > 0" :src="product['imagesResized'][1]" :alt="product['name_' + lang]")
