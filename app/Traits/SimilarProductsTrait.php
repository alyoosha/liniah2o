...
trait SimilarProductsTrait {
    public function getSimilarProducts($product_similar_array) {
        $similar_product_articules = $product_similar_array ? json_decode($product_similar_array)->ids : [];

        if(count($similar_product_articules) >= 12) {
            $similar_product_articules = array_slice($similar_product_articules, 0, 12);
        }

        $imploded_similar_product_articules = implode(',', $similar_product_articules);
                        ...
                        GROUP_CONCAT(DISTINCT (images.paths_resized) ORDER BY images.name SEPARATOR ',') AS images_resized
                            ...
                            from `products` as products
                                ...
                                left join `images` images ON images.product_articul=products.articul and images.is_visible
                                    ...
                                    where products.articul in ($imploded_similar_product_articules) and products.is_visible
